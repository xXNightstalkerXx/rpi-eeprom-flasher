...
then
	...
	echo ""
        ...
        echo "${blue}Moving the Scripts into /usr/bin Directory...${std}"
        sudo mv rpi-eeprom-flasher $eeprom_scripts_dir
        sudo mv rpi-eeprom-creator $eeprom_scripts_dir
        sudo mv rpi-eeprom-installer $eeprom_scripts_dir
        sudo mv rpi-eeprom-uninstaller $eeprom_scripts_dir
        echo "${blue}Moving the Config Files to /usr/bin/eeprom_flasher/configs ...${std}"
        sudo mv default-sd-config.txt $config_dir
        sudo mv default-usb-config.txt $config_dir
        echo "${blue}Cleaning up the /tmp/downloads Directory...${std}"
	...
	if [[ $start_dir == *"rpi-eeprom-flasher" ]]
		...
		echo "${blue}Purging the Directory now...${std}"
                ...
                echo ""
                echo "${blue}Installation Complete! Good Bye!${std}"
                sudo rm $start_dir"/rpi-eeprom-installer"
        fi
else
	echo ""
